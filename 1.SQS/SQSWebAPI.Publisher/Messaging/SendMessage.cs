...
public sealed class SendMessage(IAmazonSQS sqsClient)
{
	public async Task<SendMessageResponse> SendMessageAsync<T>(T message, CancellationToken cancellationToken = default)
	{
		var queueUrResponse = await sqsClient.GetQueueUrlAsync("customers", cancellationToken);

		var sendMessageRequest = new SendMessageRequest
		{
			QueueUrl = queueUrResponse.QueueUrl,
